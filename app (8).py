...
def dark_layout(title="", height=400, margin=None, legend=True):
    """Dark-mode Plotly layout — Claude-style dark background."""
    BG   = "#0f1117"
    SURF = "#1a1f2e"
    GRID = "#2d3748"
    TXT  = "#e2e8f0"
    ...
    m    = margin or dict(l=60, r=30, t=50 if title else 30, b=50)
    return dict(
        title=dict(text=title, font=dict(size=14, color=TXT, family=FONT),
                   x=0, xanchor="left", pad=dict(l=4)),
        height=height,
        plot_bgcolor=SURF,
        paper_bgcolor=BG,
        font=dict(family=FONT, size=12, color=TXT),
        margin=m,
        showlegend=legend,
        legend=dict(bgcolor="rgba(0,0,0,0)", borderwidth=0,
                    font=dict(size=11, color=SUBT)),
        xaxis=dict(showgrid=True, gridcolor=GRID, gridwidth=1,
                   zeroline=False, linecolor=GRID, linewidth=1,
                   tickfont=dict(size=11, color=SUBT),
                   title_font=dict(size=12, color=SUBT)),
...
def themed_layout(title="", height=400, margin=None, legend=True):
    """Return light or dark layout based on Streamlit's active theme."""
    try:
        theme = st.get_option("theme.base") or "light"
    except Exception:
        theme = "light"
    if theme == "dark":
        return dark_layout(title, height, margin, legend)
    return base_layout(title, height, margin, legend)

def apply_theme(fig, title="", height=400, margin=None, legend=True):
    fig.update_layout(**themed_layout(title, height, margin, legend))
    return fig
...
<style>
  ...
  /* ── Fallback: target Streamlit's dark stApp background directly ── */
  ...
  /* ── Named classes using variables ── */
  .main-header   {font-size:2rem;font-weight:700;color:var(--txt-primary);
                  margin-bottom:0;font-family:'Inter',sans-serif;}
  .sub-header    {font-size:0.9rem;color:var(--txt-secondary);margin-bottom:1.5rem;
                  font-family:'Inter',sans-serif;}
  .section-title {font-size:1.05rem;font-weight:600;color:var(--txt-primary);
                  border-left:3px solid var(--section-left);padding-left:0.75rem;
                  margin:1.5rem 0 0.75rem;font-family:'Inter',sans-serif;}
  .tag           {display:inline-block;background:var(--bg-tag);color:var(--txt-tag);
                  border-radius:4px;padding:2px 8px;font-size:0.75rem;
                  font-weight:500;margin:2px;font-family:'Inter',sans-serif;}
  ...
  .kpi-label     {font-size:11px;color:var(--txt-label);text-transform:uppercase;
                  letter-spacing:0.05em;font-family:'Inter',sans-serif;font-weight:500;}
  .kpi-value     {font-size:18px;font-weight:700;color:var(--txt-primary);
                  word-break:break-word;line-height:1.3;font-family:'Inter',sans-serif;}

  /* ── Tooltips ── */
  .tt-icon {display:inline-flex;align-items:center;justify-content:center;
            width:14px;height:14px;border-radius:50%;background:var(--bg-icon);
            color:var(--txt-secondary);font-size:9px;font-weight:700;cursor:default;
  ...
  .tt-box   {display:none;position:absolute;bottom:calc(100% + 6px);left:50%;
             transform:translateX(-50%);background:var(--bg-tt);color:var(--txt-tt);
  ...
  div[data-testid="stButton"] > button:first-child [data-testid="stIconMaterial"],
  div[data-testid="stButton"] > button:first-child span[aria-hidden="true"],
  div[data-testid="stButton"] > button:first-child .st-emotion-cache-1clstc5 {
    display:none !important;
  }

  /* ── Material Symbols fix ── */
  @font-face {
    font-family:'Material Symbols Rounded';font-style:normal;font-weight:100 700;
  ...
  span[data-baseweb] span[role="img"] {
    ...
    font-feature-settings:'liga' 1;-webkit-font-feature-settings:'liga' 1;
  }
</style>
""", unsafe_allow_html=True)

# Inject JS that reads Streamlit's theme and sets CSS variables at runtime
st.markdown("""
<script>
(function() {
  function applyTheme() {
    // Streamlit stores theme in localStorage and on the body/html
    const isDark =
      document.body.classList.contains('dark') ||
      document.documentElement.getAttribute('data-theme') === 'dark' ||
      window.matchMedia('(prefers-color-scheme: dark)').matches ||
      (window.__streamlit && window.__streamlit.theme && window.__streamlit.theme.base === 'dark') ||
      document.querySelector('[data-testid="stApp"]')?.style?.colorScheme === 'dark' ||
      getComputedStyle(document.documentElement).getPropertyValue('--background-color').trim() === '#0e1117' ||
      getComputedStyle(document.querySelector('[data-testid="stAppViewContainer"]') || document.body)
        .backgroundColor.includes('14, 17') ||
      getComputedStyle(document.body).backgroundColor === 'rgb(14, 17, 23)';

    const root = document.documentElement;
    if (isDark) {
      root.style.setProperty('--bg-card',       '#1e2433');
      root.style.setProperty('--bd-card',       '#2d3748');
      root.style.setProperty('--txt-primary',   '#f1f5f9');
      root.style.setProperty('--txt-secondary', '#94a3b8');
      root.style.setProperty('--txt-label',     '#94a3b8');
      root.style.setProperty('--bg-tag',        '#1e3a5f');
      root.style.setProperty('--txt-tag',       '#93c5fd');
      root.style.setProperty('--bg-tt',         '#0f172a');
      root.style.setProperty('--txt-tt',        '#e2e8f0');
      root.style.setProperty('--bg-icon',       '#334155');
    } else {
      root.style.setProperty('--bg-card',       '#f8fafc');
      root.style.setProperty('--bd-card',       '#e2e8f0');
      root.style.setProperty('--txt-primary',   '#1a1a2e');
      root.style.setProperty('--txt-secondary', '#6b7280');
      root.style.setProperty('--txt-label',     '#6b7280');
      root.style.setProperty('--bg-tag',        '#eff6ff');
      root.style.setProperty('--txt-tag',       '#1d4ed8');
      root.style.setProperty('--bg-tt',         '#1e293b');
      root.style.setProperty('--txt-tt',        '#f1f5f9');
      root.style.setProperty('--bg-icon',       '#e2e8f0');
    }
  }

  // Run immediately and again after short delays to catch Streamlit's late renders
  applyTheme();
...
def kpi_card(label, value, sub=None):
    sub_html = f"<div style='font-size:11px;color:var(--txt-secondary);margin-top:3px;font-family:Inter,sans-serif;'>{sub}</div>" if sub else ""
    tooltip  = tt(label)
    return (
        f"<div class='kpi-card-inner'>"
...
if show_bridge:
    ...
    try:
        ...
        _theme = "light"
    _conn_color = "#4b5563" if _theme == "dark" else "#cbd5e1"
    fig_bridge.update_traces(connector=dict(line=dict(color=_conn_color, width=1, dash="dot")))
...
with tab7:
    ...
    apply_theme(fig_h, height=460, legend=False)
    fig_h.update_layout(
